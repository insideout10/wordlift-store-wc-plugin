...
* A `wordlift-store` directory that contains the source code - a fully executable WordPress plugin.
...
* `wordlift-store/includes` is where functionality shared between the admin area and the public-facing parts of the site reside
* `wordlift-store/admin` is for all admin-specific functionality
* `wordlift-store/public` is for all public-facing functionality

Note that previous versions of the Boilerplate did not include `Wordlift_Store_Loader` but this class is used to register all filters and actions with WordPress.
